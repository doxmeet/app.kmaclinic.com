...
const ResizableImage = Image.extend({
	...
	addNodeView() {
		...
		return (props: Parameters<typeof parentNodeView>[0]) => {
			const nodeView = parentNodeView(props);

			// 정렬(textAlign)과 sizeStyle 을 함께 고려해 컨테이너 / wrapper / img 스타일을 결정한다.
			// - 기본(정렬 없음): inline-block / inline-flex → 같은 줄에 여러 이미지를 나열할 수 있다.
			// - center / right 정렬: block + margin auto 로 줄을 독점한다.
			const applyImageStyles = (node: PmNode) => {
				const sizeStyle = node.attrs.sizeStyle as string | null;
				const pxWidthAttr = node.attrs.width as number | string | null;
				const pxHeightAttr = node.attrs.height as number | string | null;
				const align = node.attrs.textAlign as string | null;
				const vAlign = node.attrs.verticalAlign as string | null;
				...
				// sizeStyle (%) 우선, 없으면 width/height 속성(px) 사용
				let widthVal = "";
				let heightVal = "";
				let mode: "pct" | "px" | "none" = "none";
				if (sizeStyle) {
					const widthMatch = /width\s*:\s*([^;]+)/i.exec(sizeStyle);
					const heightMatch = /height\s*:\s*([^;]+)/i.exec(sizeStyle);
					widthVal = widthMatch ? widthMatch[1].trim() : "";
					const hv = heightMatch ? heightMatch[1].trim() : "";
					heightVal = hv && !/^auto$/i.test(hv) ? hv : "";
					mode = "pct";
				} else if (pxWidthAttr != null || pxHeightAttr != null) {
					if (pxWidthAttr != null) widthVal = `${pxWidthAttr}px`;
					if (pxHeightAttr != null) heightVal = `${pxHeightAttr}px`;
					mode = "px";
				}

				const isCenter = align === "center";
				const isRight = align === "right";
				const isAligned = isCenter || isRight;
				...
				if (mode === "pct") {
					if (isAligned) {
						// block + margin auto 로 한 줄 독점 (중앙/우측 정렬)
						container.style.display = "block";
						container.style.width = widthVal || "auto";
						container.style.height = heightVal || "";
						container.style.marginLeft = "auto";
						container.style.marginRight = isCenter ? "auto" : "0";
					} else {
						// inline-block 으로 같은 줄에 다른 이미지/텍스트와 공존 가능
						container.style.display = "inline-block";
						container.style.width = widthVal || "";
						container.style.height = heightVal || "";
						container.style.verticalAlign = vAlign || "top";
					}
					// % 모드: 부모(p) 폭을 넘지 않도록 cap
					container.style.maxWidth = "100%";
					if (innerWrapper) {
						innerWrapper.style.width = "100%";
						innerWrapper.style.height = heightVal ? "100%" : "";
						innerWrapper.style.maxWidth = "100%";
					}
					img.style.width = "100%";
					img.style.height = heightVal ? "100%" : "auto";
					img.style.maxWidth = "100%";
				} else if (mode === "px") {
					// 절대 px 크기: 사용자가 입력한 값을 그대로 반영하기 위해 max-width cap 을 제거.
					// base NodeView 의 applyInitialSize 는 생성 시 1회만 실행되므로,
					// updateAttributes 후 재적용하려면 여기서 직접 img 스타일을 덮어쓴다.
					if (isAligned) {
						container.style.display = "block";
						container.style.width = widthVal || "auto";
						container.style.height = heightVal || "";
					...
					img.style.height = heightVal || (widthVal ? "auto" : "");
					...
					if (isAligned) {
						...
						// base 가 createContainer 에서 inline-flex 인라인 스타일을 걸지만,
						// 업데이트로 지워질 수 있으므로 명시적으로 복원한다.
			...
			applyImageStyles(props.node);

			const origUpdate = nodeView.update?.bind(nodeView);
			if (origUpdate) {
				nodeView.update = (
					updatedNode: PmNode,
					decorations: readonly import("@tiptap/pm/view").Decoration[],
					innerDecorations: import("@tiptap/pm/view").DecorationSource,
				) => {
					const result = origUpdate(updatedNode, decorations, innerDecorations);
					if (result !== false) {
						applyImageStyles(updatedNode);
					}
					return result;
				};
			}
