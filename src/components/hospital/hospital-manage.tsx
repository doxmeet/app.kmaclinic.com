...
type FormDispatch = React.Dispatch<FormAction>;
			...
			<FieldGroup>
				...
				<div className="grid gap-6 sm:grid-cols-2">
					...
					<Field>
						...
						<FieldSelect
							value={state.year}
							onValueChange={(v) => dispatch({ type: "setYear", value: v })}
				...
				<Field>
					...
					<FieldRow className="flex-wrap">
						<FieldSelect
							value={state.day}
							onValueChange={(v) => dispatch({ type: "setDay", value: v })}
							options={DAY_OPTIONS}
						...
						<div className="flex flex-1 items-center gap-3">
							<FieldSelect
								...
								onValueChange={(v) =>
									dispatch({ type: "setOpenTime", value: v })
								}
							...
							<FieldSelect
								...
								onValueChange={(v) =>
									dispatch({ type: "setCloseTime", value: v })
								}
								options={TIME_OPTIONS}
							/>
						</div>
					...
					<div className="flex flex-col gap-3 rounded-lg border border-line-soft bg-app-bg p-4 sm:flex-row sm:items-center">
						<span className="flex items-center gap-2 text-base text-ink">
							<Switch
								...
								onCheckedChange={(v) =>
									dispatch({ type: "setLunchEnabled", value: v })
								}
						...
						{state.lunchEnabled ? (
							<div className="sm:flex-1">
								<FieldSelect
									value={state.lunch}
									onValueChange={(v) =>
										dispatch({ type: "setLunch", value: v })
									}
					...
					<Field>
						...
						<FieldRow className="flex-wrap">
							<FieldSelect
								...
								onValueChange={(v) =>
									dispatch({ type: "setSubwayLine", value: v })
								}
							...
							<FieldSelect
								value={state.walk}
								onValueChange={(v) => dispatch({ type: "setWalk", value: v })}
	...
	return (
		<SectionCard>
			...
			<div className="flex flex-col gap-5">
				...
				>
					연동 실패 : 네이버 플레이스 데이터가 추출되지 않았습니다. 다시 시도해
					주세요.
				</InfoCallout>
				...
				<div className="flex flex-col gap-2">
					...
					<div className="rounded-lg border border-line-soft bg-app-bg px-4 py-3.5 text-[17px] text-body">
						버스 이용시: 간선 130, 341, 370 / 지선 3214, 3316, 3411 ('천호역
						현대백화점' 정류장 하차)
					</div>
				</div>
		...
		<AppShell userName="김민준 원장" maxWidth="1280px">
			<div className="flex flex-col gap-6 lg:flex-row lg:gap-8">
				...
				<div className="flex min-w-0 flex-1 flex-col gap-8">
					<PageHeader />
					<HospitalImageSection />
					<HospitalInfoSection state={state} dispatch={dispatch} />
					<DoctorsSection />
					<LocationSection state={state} dispatch={dispatch} />
					<NaverSyncSection />
					<NonCoveredSection />
					<DesignSection state={state} dispatch={dispatch} />
