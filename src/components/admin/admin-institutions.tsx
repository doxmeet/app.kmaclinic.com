...
type BillingValue = (typeof BILLING_CYCLE_FILTERS)[number]["value"];
...
type InputAction =
	...
	| { type: "setBilling"; value: BillingValue }
...
function appliedReducer(
	...
	switch (action.type) {
		case "apply":
			return {
				keyword: action.value.keyword.trim(),
				status: action.value.status,
				billingCycle: action.value.billing,
				dateFrom: action.value.dateFrom,
				dateTo: action.value.dateTo,
				page: 1,
			};
		case "reset":
					...
					<div className="flex flex-col gap-2">
						...
						<div className="relative">
							...
							<Input
								value={input.keyword}
								onChange={(e) =>
									dispatchInput({ type: "setKeyword", value: e.target.value })
								}
								onKeyDown={(e) => {
...
function SubscriptionRow({ row }: { row: AdminSubscription }) {
	...
	return (
		<TableRow className="border-b-line-strong/50">
			<TableCell>
				<span className="text-[15px] text-ink">{name}</span>
			</TableCell>
			<TableCell className="text-[15px] text-ink">{owner}</TableCell>
			<TableCell>
			...
			<TableCell className="max-w-[200px] truncate text-body">{memo}</TableCell>
			<TableCell>
...
function InstitutionsPage() {
	...
	const { keyword, status, billingCycle, dateFrom, dateTo, page } = applied;
	...
	const runSearch = () => {
		dispatchApplied({ type: "apply", value: input });
	};
		...
		<AdminShell active="institutions">
			<div className="flex flex-col gap-8">
				...
				<ResultsSection
					...
					totalPages={totalPages}
					onPageChange={(next) =>
						dispatchApplied({ type: "setPage", value: next })
					}
				/>
			</div>
		</AdminShell>
